  ...
  return (
    <main className="container">
      ...
      <form onSubmit={submit} className="card">
        ...
        <Field id="organization" label={t("auth.signup.organization")} value={state.organization} onChange={state.setOrganization} />
        ...
        <button type="submit" className="btn" disabled={state.busy}>{state.busy ? t("auth.signup.creating") : t("auth.signup.action")}</button>
      </form>
      <p className="muted">{t("auth.signup.hasAccount")} <Link href="/login">{t("auth.signup.login")}</Link></p>
    </main>
  );
}
