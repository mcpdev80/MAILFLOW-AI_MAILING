...
export function OnboardingUi({
  ...
  if (controller.step === "behavior")
    return <Behavior controller={controller} />;
  if (controller.step === "existing")
    return <ExistingMail controller={controller} />;
...
function Welcome({ controller }: { controller: OnboardingController }) {
  return (
    <WizardShell
      ...
      subtitle="A few quick steps will connect your mailbox and apply privacy-safe defaults."
      next={{
        label: "Connect your mailbox",
        onClick: () => controller.setStep("mailbox"),
      }}
    >
      ...
      <div className={w.info}>
        <span className={w.infoIcon}>i</span>
        <span>Safe by design. Mailflow never automatically deletes mail or sends replies.</span>
      </div>
    </WizardShell>
  );
}
...
function Mailbox({ controller }: { controller: OnboardingController }) {
  ...
  return (
    <WizardShell
      ...
      subtitle="Link your mail provider to enable Mailflow intelligence."
      back={{ onClick: () => controller.setStep("welcome") }}
      next={{ label: "Continue", onClick: controller.continueFromMailbox, disabled: controller.busy }}
    >
      <div className={styles.providerGrid}>
        {(["gmail", "microsoft", "imap"] as const).map((provider) => (
          <button
            key={provider}
            type="button"
            className={`${styles.provider} ${choice === provider ? styles.providerActive : ""}`}
            onClick={() => controller.setProviderChoice(provider)}
          >
            <span className={styles.providerIcon}>{provider === "gmail" ? "G" : provider === "microsoft" ? "M" : "✉"}</span>
            <span>{provider === "gmail" ? "Gmail" : provider === "microsoft" ? "Microsoft" : "IMAP / SMTP"}</span>
          </button>
        ))}
      </div>

      {choice === "imap" ? (
        <ImapConnection controller={controller} />
      ) : (
    ...
    <div className={styles.connectionCard}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 140px", gap: 12 }}>
        <label className={w.field}>
          IMAP host
          <input value={form.imap_host} onChange={(e) => update("imap_host", e.target.value)} placeholder="imap.example.com" />
        </label>
        <label className={w.field}>
          Port
          <input type="number" min={1} max={65535} value={form.imap_port} onChange={(e) => update("imap_port", Number(e.target.value))} />
        </label>
      </div>
      <label className={w.field}>
        Security
        <select value={form.use_ssl ? "ssl" : "plain"} onChange={(e) => update("use_ssl", e.target.value === "ssl")}>
      ...
      <label className={w.field}>
        Password
        <input type="password" value={form.password} onChange={(e) => update("password", e.target.value)} autoComplete="current-password" />
      </label>

      <div style={{ borderTop: "1px solid var(--mf-border)", marginTop: 6, paddingTop: 18 }}>
        <h3 style={{ marginTop: 0 }}>Outgoing mail (SMTP)</h3>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 140px", gap: 12 }}>
          <label className={w.field}>
            SMTP host
            <input value={form.smtp_host} onChange={(e) => update("smtp_host", e.target.value)} placeholder="smtp.example.com" />
          </label>
          <label className={w.field}>
            Port
            <input type="number" min={1} max={65535} value={form.smtp_port} onChange={(e) => update("smtp_port", Number(e.target.value))} />
          </label>
        </div>
        <label className={w.field}>
          Security
          <select value={form.smtp_security} onChange={(e) => update("smtp_security", e.target.value as typeof form.smtp_security)}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
          <input
            type="checkbox"
            checked={form.smtp_same_credentials}
            onChange={(e) => update("smtp_same_credentials", e.target.checked)}
            style={{ width: 16, minHeight: 16 }}
          />
          Use the same username and password as IMAP
        </label>
        {!form.smtp_same_credentials && (
          <>
            <label className={w.field}>
              SMTP username
              <input value={form.smtp_username} onChange={(e) => update("smtp_username", e.target.value)} autoComplete="username" />
            </label>
            <label className={w.field}>
              SMTP password
              <input type="password" value={form.smtp_password} onChange={(e) => update("smtp_password", e.target.value)} autoComplete="current-password" />
            </label>
          </>
        )}
      </div>
    ...
    <WizardShell kind="onboarding" step={3} total={6} title="Mailbox privacy" subtitle="Who should have access to this mailbox?" back={{ onClick: () => controller.setStep("mailbox") }} next={{ label: controller.busy ? "Saving…" : "Continue", onClick: () => void controller.savePrivacyAndMailbox(), disabled: controller.busy }}>
      <div className={styles.radioCards}>
        <button type="button" className={`${styles.radioCard} ${form.ownership_mode === "private" ? styles.radioCardActive : ""}`} onClick={() => controller.setAccountForm({ ...form, ownership_mode: "private", shared_user_ids: [] })}>
          ...
          <span><span className={styles.radioTitle}>⌁ Private (Recommended)</span><span className={styles.radioCopy}>Only you can access this mailbox and its Mailflow intelligence reports.</span></span>
        </button>
        <button type="button" disabled={!controller.canCreateShared} className={`${styles.radioCard} ${form.ownership_mode === "shared" ? styles.radioCardActive : ""}`} onClick={() => controller.setAccountForm({ ...form, ownership_mode: "shared" })}>
          <span className={styles.radioDot} />
          <span><span className={styles.radioTitle}>◉ Shared Mailbox</span><span className={styles.radioCopy}>Grant custom read/write workspace access to selected team members.</span></span>
        </button>
      </div>
...
function MemberOption({ member, controller }: { member: OrganizationMember; controller: OnboardingController }) {
  ...
  return (
    <label className={styles.member}>
      <input type="checkbox" checked={controller.accountForm.shared_user_ids.includes(userId)} onChange={(event) => controller.toggleSharedUser(userId, event.target.checked)} />
      <span>{label} · {member.role}</span>
    </label>
  );
}
...
function Behavior({ controller }: { controller: OnboardingController }) {
  const form = controller.accountForm;
  return (
    <WizardShell kind="onboarding" step={4} total={6} title="Mailflow behavior" subtitle="Choose how Mailflow should handle mailbox actions. Core classification and safety protections stay enabled." back={{ onClick: () => controller.setStep("privacy") }} next={{ label: controller.busy ? "Saving…" : "Continue", onClick: () => void controller.saveBehavior(), disabled: controller.busy }}>
      <div className={styles.behaviorRows}>
        <BehaviorRow title="Classification" copy="Categorize incoming email automatically" control={<span className={styles.policy}>Always enabled</span>} />
        <BehaviorRow title="Tags" copy="Apply system urgency & action tags" control={<span className={styles.policy}>Always enabled</span>} />
        <BehaviorRow title="Move to folders" copy="Choose whether safe folder moves run automatically or require review" control={<select className={styles.policy} aria-label="Move to folders policy" value={form.move_policy} onChange={(event) => controller.setAccountForm({ ...form, move_policy: event.target.value as typeof form.move_policy })}><option value="off">Off</option><option value="review">Review first</option><option value="automatic">Automatic when safe</option></select>} />
        <BehaviorRow title="Archive" copy="Choose whether processed email may be archived automatically" control={<select className={styles.policy} aria-label="Archive policy" value={form.archive_policy} onChange={(event) => controller.setAccountForm({ ...form, archive_policy: event.target.value as typeof form.archive_policy })}><option value="off">Off</option><option value="review">Review first</option><option value="automatic">Automatic when safe</option></select>} />
...
function ExistingMail({ controller }: { controller: OnboardingController }) {
  return (
    <WizardShell kind="onboarding" step={5} total={6} title="Existing email" subtitle="Should Mailflow analyze your existing messages?" back={{ onClick: () => controller.setStep("behavior") }} next={{ label: controller.busy ? "Starting…" : "Continue", onClick: () => void controller.finishExisting(), disabled: controller.busy }}>
      ...
      <div className={styles.connectionCard}><h3>ⓘ How Dry Run works</h3><p>Mailflow will analyze your existing email in the background using Dry Run mode — it proposes changes without modifying your mailbox. You review and approve before anything changes.</p></div>
