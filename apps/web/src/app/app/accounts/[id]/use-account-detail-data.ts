...
export function useAccountDetailData(id: string, userId?: string) {
  ...
  const load = useCallback(async () => {
    ...
    await loadAccountData(id, resolved.account, resolved.contentAccessible, {
      setCycles, setSharedAccess, setSelectedSharedUsers, setError,
    });
  }, [id]);
  ...
  return {
    account, setAccount, contentAccessible, setContentAccessible, cycles, setCycles,
    memberOptions, sharedAccess, setSharedAccess, selectedSharedUsers, setSelectedSharedUsers,
    transferUserId, setTransferUserId, error, setError, notice, setNotice, busy, setBusy,
    permissions, totals, reload: load,
  };
}
...
async function loadSharedAccess(
  ...
  try {
    ...
    setters.setSelectedSharedUsers(access.filter((grant) => grant.can_use).map((grant) => grant.user_id));
  } catch (err) {
    if (err instanceof ApiError && err.status === 404) setters.setSharedAccess(null);
