...
export function DecisionMemoryUi(props: DecisionMemoryUiProps) {
  ...
  return (
    <main className="container" style={{ maxWidth: 1440, margin: "0 auto" }}>
      ...
      {props.error && (
        <ErrorBanner error={props.error} onReload={props.onReload} />
      )}
      {props.notice && (
...
function DecisionCard(
  props: DecisionMemoryUiProps & { entry: DecisionMemoryEntry },
) {
  const isEditing = props.editing === props.entry.id && props.draft !== null;
  return (
    <article
      className="card"
      style={{ opacity: props.entry.enabled ? 1 : 0.68 }}
    >
      <DecisionCardHeader {...props} isEditing={isEditing} />
      {isEditing && props.draft ? (
        <DecisionEditor {...props} draft={props.draft} />
      ) : (
        <DecisionSummary entry={props.entry} />
      )}
    </article>
  );
}

function DecisionCardHeader(
  props: DecisionMemoryUiProps & {
      ...
      <div style={{ minWidth: 0 }}>
        ...
        >
          <h3 style={{ margin: 0, overflowWrap: "anywhere" }}>
            {decisionMatchLabel(entry)}
          </h3>
          <span className={`pill ${entry.enabled ? "ok" : "off"}`}>
            {entry.enabled ? t("decision.enabled") : t("decision.disabled")}
          </span>
          {entry.superseded_by_id && (
            <span className="pill off">{t("decision.superseded")}</span>
          )}
        </div>
        <div className="muted" style={{ marginTop: 6 }}>
          {t("decision.source")}: {entry.source.replaceAll("_", " ")} ·{" "}
          {t("decision.trust")}: {Math.round(entry.trust_score * 100)}% ·{" "}
          {t("decision.used")}: {entry.hit_count}×
          {entry.last_used
            ? ` · ${t("decision.lastUsed")}: ${new Date(entry.last_used).toLocaleString(locale)}`
            : ""}
        </div>
      </div>
...
function DecisionActions(
  ...
  async function remove() {
    if (window.confirm(t("decision.deleteConfirm")))
      await props.onRemove(props.entry);
  }
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
      <button
        ...
        type="button"
        disabled={busy}
        onClick={() => props.onBeginEdit(props.entry)}
      >
        {t("common.edit")}
      </button>
      <button
        ...
        type="button"
        disabled={busy}
        onClick={() => void props.onToggle(props.entry)}
      >
        {props.entry.enabled ? t("common.disable") : t("common.enable")}
      </button>
      <button
...
function DecisionSummary({ entry }: { entry: DecisionMemoryEntry }) {
  ...
  const values = [
    [
      t("decision.classification"),
      `${enumLabel(t, "category", entry.category)}${entry.subcategory ? ` / ${entry.subcategory}` : ""}`,
    ],
    [
      t("decision.priority"),
      `${enumLabel(t, "importance", entry.importance)} · ${enumLabel(t, "urgency", entry.urgency)}`,
    ],
    [
      t("decision.action"),
      enumLabel(t, "action_required", entry.action_required),
    ],
...
function DecisionEditor(
  props: DecisionMemoryUiProps & {
    entry: DecisionMemoryEntry;
    draft: DecisionMemoryWrite;
  },
) {
  const { t } = useI18n();
  const patch = (value: Partial<DecisionMemoryWrite>) =>
    props.setDraft({ ...props.draft, ...value });
  return (
    <div style={{ marginTop: 18 }}>
      <div className="row">
        <SelectField
          ...
          onChange={(category) =>
            patch({ category: category as DecisionMemoryWrite["category"] })
          }
        />
        <TextField
          label={t("decision.subcategory")}
      ...
      <div className="row">
        <SelectField
          label={t("decision.importance")}
          value={props.draft.importance}
          options={importanceValues}
          labelFor={(v) => enumLabel(t, "importance", v)}
          onChange={(importance) =>
            patch({
              importance: importance as DecisionMemoryWrite["importance"],
            })
          }
        />
        <SelectField
          label={t("decision.urgency")}
          value={props.draft.urgency}
          options={urgencyValues}
          labelFor={(v) => enumLabel(t, "urgency", v)}
          onChange={(urgency) =>
            patch({ urgency: urgency as DecisionMemoryWrite["urgency"] })
          }
        />
        <SelectField
          label={t("decision.actionRequired")}
          value={props.draft.action_required}
          options={actionValues}
          labelFor={(v) => enumLabel(t, "action_required", v)}
          onChange={(action_required) =>
            patch({
              action_required:
                action_required as DecisionMemoryWrite["action_required"],
            })
          }
        />
      </div>
      <div className="row">
        <TextField
          label={t("decision.subjectPattern")}
          value={props.draft.subject_pattern ?? ""}
          onChange={(subject_pattern) =>
            patch({ subject_pattern: subject_pattern || null })
          }
        />
        <TextField
          label={t("decision.routingTarget")}
          value={props.draft.routing_target ?? ""}
          onChange={(routing_target) =>
            patch({ routing_target: routing_target || null })
          }
        />
      </div>
      <EditorActions {...props} />
  ...
  return (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
      <button
        ...
        disabled={busy}
        onClick={() =>
          void props.onSave(props.entry.id, {
            ...props.draft,
            source: "human_corrected",
          })
        }
      >
        {t("decision.saveCorrection")}
      </button>
      <button
        className="btn secondary"
        type="button"
