...
export function AccountDetailUi({ controller }: { controller: Controller }) {
  ...
  return (
    <main className="container" style={{ maxWidth: 1440, margin: "0 auto" }}>
      <p>
        <Link href="/app/dashboard">← {t("account.back")}</Link>
      </p>
      {error && <div className="alert error">{error}</div>}
      {notice && <div className="alert ok">{notice}</div>}
      {controller.account && (
        <AccountContent controller={controller} account={controller.account} />
      )}
    </main>
  );
}
...
function AccountContent({
  ...
  return (
    <>
      <AccountHeader controller={controller} account={account} />
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {controller.contentAccessible && (
            <Link
              ...
              href={`/app/accounts/${controller.id}/decision-memory`}
            >
              {t("account.learned")}
            </Link>
          )}
          {controller.contentAccessible && (
            ...
            >
              {controller.busy ? t("account.working") : t("account.run")}
            </button>
          )}
          {controller.permissions.canManageOwnership && (
            <button
...
function PrivateAccess({ controller }: { controller: Controller }) {
  ...
  async function transfer() {
    if (window.confirm(t("account.transferConfirm"))) {
      await controller.transferPrivateMailbox();
    }
  }
  return (
  ...
  return (
    <fieldset className="field" style={{ border: 0, padding: 0 }}>
      ...
      {controller.memberOptions.map((member) => (
        <label key={member.id} style={{ display: "flex", gap: 8 }}>
          <input
            type="checkbox"
...
function CycleOverview({ controller }: { controller: Controller }) {
  const { t } = useI18n();
  const stats = [
    [t("account.cycles"), controller.cycles.length],
    [t("account.emailsProcessed"), controller.totals.emails],
    [t("account.draftsSaved"), controller.totals.drafts],
    [t("account.errors"), controller.totals.errors],
  ] as const;
  return (
    <section>
