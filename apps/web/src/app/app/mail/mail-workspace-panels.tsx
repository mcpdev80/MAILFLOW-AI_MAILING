...
export function AccountsPanel({ state }: { state: WorkspaceState }) {
  ...
  return (
    <div className={styles.sidePane}>
      ...
      <button type="button" className={`${styles.accountButton} ${state.accountFilter === "all" ? styles.accountActive : ""}`} onClick={() => state.changeAccount("all")}>
        <span>{t("mail.all")}</span><span className={styles.count}>{state.inbox?.total_unread ?? 0}</span>
      </button>
      {state.accounts.map((account) => (
        <button type="button" key={account.id} className={`${styles.accountButton} ${state.accountFilter === account.id ? styles.accountActive : ""}`} onClick={() => state.changeAccount(account.id)}>
          <span className={styles.accountMeta}><strong>{account.username}</strong><small>{account.ownership_mode}</small></span>
...
export function FoldersPanel({ state }: { state: WorkspaceState }) {
  ...
  return (
    <div className={styles.sidePane}>
      ...
      {state.selectableFolders.map((folder) => <FolderButton key={folder.name} state={state} folder={folder} />)}
    </div>
  );
}
...
export function ClassicSidePane({ state }: { state: WorkspaceState }) {
  ...
  return (
    <aside className={styles.sidePane}>
      <select className={styles.accountSelect} value={state.accountFilter} onChange={(event) => state.changeAccount(event.currentTarget.value)} aria-label={t("mail.allMailboxes")}>
...
function FolderButton({ state, folder }: { state: WorkspaceState; folder: WorkspaceState["selectableFolders"][number] }) {
  return (
    <button
      ...
      onDrop={(event) => { event.preventDefault(); void state.dropMessagesIntoFolder(folder.name); }}
    >
      <span>{folder.name}</span>{folder.role && <span className={styles.count}>{folder.role}</span>}
    </button>
  );
}
  ...
  return (
    <section className={styles.listPane}>
      <div className={styles.listHeader}>
        <div className={styles.searchWrap}><input className={styles.searchInput} value={query} onChange={(event) => setQuery(event.currentTarget.value)} placeholder={t("nav.search")} aria-label={t("nav.search")} /></div>
        <button type="button" className={styles.refreshButton} onClick={() => void state.loadInbox()}>{t("mail.refresh")}</button>
      </div>
      <div className={styles.messageList}>
        ...
        {!state.loading && messages.map((message) => <MessageRow key={messageKey(message)} state={state} message={message} />)}
      </div>
    </section>
  );
}
...
function MessageRow({ state, message }: { state: WorkspaceState; message: InboxMessage }) {
  ...
  return (
    <button
      ...
      draggable aria-pressed={state.selectedKeys.has(key)}
      ...
      onDragEnd={() => { state.setDragMessages([]); state.setDragTarget(null); }}
