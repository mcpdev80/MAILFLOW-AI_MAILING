...
export function useMailMessageView(options: MessageViewOptions) {
  ...
  const openMessage = useCallback(
    async (message: InboxMessage) => {
      ...
      try {
        ...
        const resolved = await markReadIfNeeded(detail, metadata, options.setInbox);
  ...
  useEffect(() => {
    ...
    if (openedDeepLink.current || options.loading || !options.inbox || !target.uid || !target.account) return;
    const message = options.inbox.messages.find((item) => matchesTarget(item, target));
    if (!message) return;
...
function updateUnreadState(current: UnifiedInbox | null, detail: MessageDetail): UnifiedInbox | null {
...
async function loadThread(detail: MessageDetail): Promise<ThreadView | null> {
  return detail.thread_id ? api.threadDetail(detail.account_id, detail.thread_id) : null;
}

function matchesTarget(message: InboxMessage, target: MailDeepLinkTarget): boolean {
  return message.account_id === target.account && message.uid === target.uid && (!target.folder || message.folder === target.folder);
}
