  ...
  async function openContextMenuAt(
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : "Could not load message actions",
      );
    }
  }
  ...
  async function dropMessagesIntoFolder(destination: string) {
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : "Could not move all selected messages",
      );
      await loadInbox();
    } finally {
      setDragMessages([]);
  ...
  return (
    <main className="mailApp">
            ...
            {!loading &&
              inbox?.messages.map((message) => (
                <button
                  ...
                  className={`messageRow ${!message.seen ? "unread" : ""} ${selected && messageKey(selected) === messageKey(message) ? "selected" : ""} ${selectedKeys.has(messageKey(message)) ? "batchSelected" : ""}`}
                  draggable
      ...
      <style jsx>{`
        ...
        .messageRow.batchSelected { outline: 2px solid var(--primary); outline-offset: -2px; }
