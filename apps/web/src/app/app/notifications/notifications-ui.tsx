...
export function NotificationsUi(props: NotificationsUiProps) {
  ...
  return (
    <main className="container" style={{ maxWidth: 1440, margin: "0 auto" }}>
      ...
      {props.preferences && (
        <PreferencesCard {...props} preferences={props.preferences} />
      )}
      <section>
        <h2>
          {t("notifications.inbox")}{" "}
          {props.center
            ? `(${props.center.unread} ${t("notifications.unread")})`
            : ""}
        </h2>
        {!props.center && !props.error && (
          <div className="card muted">{t("common.loading")}</div>
        )}
        {props.center?.notifications.length === 0 && (
    ...
    <section className="card" style={{ marginBottom: 24 }}>
      ...
      >
        {booleanPreferences.map((key) => (
          <ToggleRow
            ...
            onChange={(checked) => props.onPatchPreferences({ [key]: checked })}
          />
        ))}
      </div>
      <div
      ...
      >
        <NumberPreference
          ...
          onPatch={props.onPatchPreferences}
        />
        <TimezonePreference
          preferences={props.preferences}
          onPatch={props.onPatchPreferences}
...
function NumberPreference({
  ...
  onPatch: (patch: Partial<NotificationPreference>) => void;
}) {
  const { t } = useI18n();
  return (
    <label>
      ...
      <input
        ...
        onChange={(event) =>
          onPatch({ daily_summary_hour: Number(event.target.value) })
        }
        style={{ width: "100%" }}
      />
    </label>
