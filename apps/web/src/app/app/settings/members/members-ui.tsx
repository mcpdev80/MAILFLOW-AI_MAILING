...
export function MembersUi({ controller }: { controller: MembersController }) {
  ...
  return (
    <main className="container" style={{ maxWidth: 1100, margin: "0 auto" }}>
      <p>
        <Link href="/app/dashboard">← {t("members.back")}</Link>
      </p>
      <h1>{t("members.title")}</h1>
      {controller.error && (
        <div className="alert error">{membersError(controller.error, t)}</div>
      )}
      {controller.loading ? (
        <div className="card muted">{t("common.loading")}</div>
      ) : (
        <MembersContent controller={controller} />
      )}
    </main>
  );
}
  ...
  return (
    <>
      ...
      {controller.invitations.length > 0 && (
        <InvitationList invitations={controller.invitations} />
      )}
    </>
  );
}
    ...
    <section className="card">
      ...
      {members.length === 0 ? (
        ...
        <ul>
          {members.map((member) => (
            <li key={member.id}>
              {member.user?.email ?? member.user?.name ?? member.id}{" "}
              <span className="muted">· {roleLabel(member.role, t)}</span>
            </li>
          ))}
        </ul>
      )}
    ...
    <section className="card">
      ...
      >
        ...
        <select
          ...
          onChange={(event) =>
            controller.setRole(event.target.value as "member" | "admin")
          }
        >
        ...
        <button className="btn" type="submit" disabled={controller.busy}>
          {controller.busy ? t("members.inviting") : t("members.invite")}
        </button>
      </form>
    </section>
  );
