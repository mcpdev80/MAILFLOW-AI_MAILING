...
function ThemeChoice({
  ...
  selected,
  onSelect,
}: { value: Theme; selected: boolean; onSelect: (value: Theme) => void }) {
  const { t } = useI18n();
  const preview =
    value === "light"
      ? styles.lightPreview
...
function OptionButton({
  ...
  return (
    <button
      type="button"
      className={`${styles.optionButton} ${compact ? styles.alignmentButton : ""} ${selected ? styles.selected : ""}`}
      onClick={onClick}
    >
      {label}
    </button>
...
function AppearancePanel() {
  ...
  return (
    <section className={styles.panel}>
      ...
      {form.notice && (
        <p
          className={`${styles.notice} ${form.notice === "saved" ? styles.success : styles.error}`}
        >
