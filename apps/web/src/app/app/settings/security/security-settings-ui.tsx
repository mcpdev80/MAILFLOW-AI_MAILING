...
export function SecuritySettingsUi({
  controller,
  ...
  return (
    <main className="container" style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      {controller.error && (
        <div className="alert error">{securityError(controller.error, t)}</div>
      )}
...
function RecentAuthNotice() {
  ...
  return (
    <div className="alert">
      {t("security.recentAuth")}{" "}
      <Link href="/login?redirect=/app/settings/security">
    ...
    <section className="card">
      ...
      >
        <label className="field">
          <span>{t("security.deviceName")}</span>
          <input
...
function PasskeyList({
  ...
  return (
    <section className="card">
      ...
      ) : controller.passkeys.length === 0 ? (
        <p className="muted">{t("security.empty")}</p>
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
          {controller.passkeys.map((passkey) => (
            <PasskeyCard
              key={passkey.id}
              passkey={passkey}
...
function PasskeyCard({
  passkey,
  controller,
}: { passkey: UserPasskey; controller: SecuritySettingsController }) {
  const { t, locale } = useI18n();
  const name = passkey.name || t("security.unnamed");
  async function rename() {
    const next = window
      .prompt(t("security.renamePrompt"), passkey.name ?? "")
      ?.trim();
  ...
  return (
    <article className="card" style={{ margin: 0 }}>
      <strong>{name}</strong>
      <p className="muted">
        {t("security.created")}: {formatDate(passkey.createdAt, locale)} ·{" "}
        {t("security.type")}: {passkey.deviceType || "—"}
        {passkey.backedUp ? ` · ${t("security.backedUp")}` : ""}
      </p>
...
function securityError(
  ...
  if (value === "security_delete_failed" || value === "security_update_failed")
    return t("security.deleteFailed");
  return value;
}
