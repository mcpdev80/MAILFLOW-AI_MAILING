...
export default function LearningPage() {
  ...
  const active = entries.filter(
    (entry) => entry.enabled && !entry.superseded_by_id,
  ).length;
  ...
  async function toggleEnabled(entry: MemoryWithMailbox) {
    setError(null);
    try {
      await api.updateDecisionMemory(
        entry.account_id,
        entry.id,
        toWrite(entry, !entry.enabled),
      );
      await load();
    } catch (err) {
  ...
  return (
    <main className={page.page}>
      ...
      <div className={page.kpis}>
        <Kpi
          label={t("learning.activeRules")}
          value={String(active)}
          meta={t("learning.trustedMemory")}
        />
        <Kpi
          label={t("learning.disabled")}
          value={String(disabled)}
          meta={t("learning.notReused")}
        />
        <Kpi
          label={t("learning.reusedWeek")}
          value="—"
          meta={t("learning.needsMetric")}
        />
      </div>
      ...
      <div className={page.toolbar}>
        <input
          ...
          placeholder={t("learning.filterPlaceholder")}
        />
        <select
          style={{ width: 190 }}
          value={source}
        ...
        >
          ...
          <option value="human_confirmed">
            {t("learning.humanConfirmed")}
          </option>
          <option value="human_corrected">
            {t("learning.humanCorrected")}
          </option>
      ...
      {loading ? (
        <div className={page.empty}>{t("learning.loading")}</div>
      ) : filtered.length === 0 ? (
        <div className={page.empty}>{t("learning.noMatches")}</div>
      ) : (
        <div className={page.list}>
          {filtered.map((entry) => (
            <article
              key={entry.id}
              className={`${page.card} ${expanded === entry.id ? page.cardSelected : ""}`}
            >
              <button
                type="button"
                style={{
              ...
              >
                <div className={page.cardHeader}>
                  <div className={page.cardTitleGroup}>
                    ...
                    <span className={page.cardTitle}>
                      {memoryTitle(entry, t)}
                    </span>
                  </div>
                  <span className={trustClass(entry.trust_score)}>
                    {trustLabel(entry.trust_score, t)}
                  </span>
                </div>
                <div className={page.meta}>
                  <span>
                    {Math.round(entry.trust_score * 100)}% {t("learning.trust")}
                  </span>
                  <span className={page.metaSep} />
                  <span>
                    {entry.hit_count} {t("learning.timesUsed")}
                  </span>
                  <span className={page.metaSep} />
                  <span>{entry.mailboxLabel}</span>
                  <span className={page.metaSep} />
                  <span>
                    {t("learning.updated")} {formatTime(entry.updated_at)}
                  </span>
                </div>
              </button>
              {expanded === entry.id && (
                <div className={page.detail}>
                  <div>
                    <strong>{t("learning.match")}:</strong>{" "}
                    {matchDescription(entry)}
                  </div>
                  <div>
                    <strong>{t("learning.decision")}:</strong> {entry.category}
                  ...
                  {entry.routing_target && (
                    <div>
                      <strong>{t("learning.routing")}:</strong>{" "}
                      {entry.routing_target}
                    </div>
                  )}
                  <div className={page.actions}>
                    ...
                    >
                      {entry.enabled
                        ? t("learning.disableAutomation")
                        : t("learning.enableAutomation")}
                    </button>
                  </div>
                </div>
              )}
  ...
  return (
    <div className={page.kpi}>
      <div className={page.kpiLabel}>
        <span>{label}</span>
        <span className={page.kpiDot} />
      </div>
      <div className={page.kpiPrimary}>
        <span className={page.kpiValue}>{value}</span>
        <span className={page.kpiMeta}>{meta}</span>
      </div>
    </div>
  );
}
...
function SourceBadge({
  source,
  t,
}: { source: DecisionMemoryEntry["source"]; t: Translator }) {
  const cls =
    source === "human_confirmed"
      ? page.success
...
function CreateRuleModal({
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!form.sender_email && !form.sender_domain && !form.thread_id)
        throw new Error(t("learning.matcherRequired"));
    ...
    >
      <dialog open className={page.modal} aria-labelledby="create-memory-title">
        ...
        <form className={page.modalForm} onSubmit={submit}>
          ...
          <label className="field">
            {t("learning.senderDomain")}
            <input
              value={form.sender_domain ?? ""}
              onChange={(e) =>
                setForm({ ...form, sender_domain: e.target.value || null })
              }
              placeholder="example.com"
            />
          </label>
          <label className="field">
            {t("learning.senderEmail")}
            <input
              type="email"
              value={form.sender_email ?? ""}
              onChange={(e) =>
                setForm({ ...form, sender_email: e.target.value || null })
              }
          ...
          <label className="field">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  category: e.target.value as DecisionMemoryWrite["category"],
                })
              }
            >
              {[
          ...
          <label className="field">
            {t("learning.routingTarget")}
            <input
              value={form.routing_target ?? ""}
              onChange={(e) =>
                setForm({ ...form, routing_target: e.target.value || null })
              }
              placeholder={t("learning.optionalFolder")}
            />
          </label>
          {error && <div className={page.error}>{error}</div>}
          <div className={page.modalActions}>
            ...
            <button className="btn" type="submit" disabled={busy}>
              {busy ? t("learning.creating") : t("learning.confirm")}
            </button>
          </div>
        </form>
      </dialog>
...
function toWrite(
  ...
  return {
    ...
    source: entry.source === "ai_observed" ? "human_confirmed" : entry.source,
...
function memoryTitle(entry: DecisionMemoryEntry, t: Translator): string {
  ...
  if (entry.sender_email)
    return `${t("learning.messagesFrom")} ${entry.sender_email}`;
  if (entry.sender_domain)
    return `${t("learning.messagesFrom")} ${entry.sender_domain}`;
  if (entry.thread_id) return `${t("learning.thread")} ${entry.thread_id}`;
  return `${entry.category} ${t("learning.decisionSuffix")}`;
}
function matchDescription(entry: DecisionMemoryEntry): string {
  return [
    entry.sender_email && `sender ${entry.sender_email}`,
    entry.sender_domain && `domain ${entry.sender_domain}`,
    entry.subject_pattern && `subject ${entry.subject_pattern}`,
    entry.thread_id && `thread ${entry.thread_id}`,
  ]
    .filter(Boolean)
    .join(" · ");
}
function trustLabel(score: number, t: Translator): string {
  return score >= 0.9
    ? t("learning.highTrust")
    : score >= 0.75
      ? t("learning.mediumTrust")
      : t("learning.evaluating");
}
