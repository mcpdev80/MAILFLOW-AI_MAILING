...
export function SearchFiltersPanel(props: {
  ...
  const active = Object.entries(props.filters)
    ...
    <section className={styles.searchCard}>
      ...
      <div className={styles.activeFilters}>
        ...
        {active.slice(0, 5).map((item) => (
          <span className={styles.filterChip} key={item}>
            {item}
          </span>
        ))}
        <button
        ...
        >
          {open ? "Hide filters" : "+ Add more filters"}
        </button>
      </div>
      {open && (
        <Filters
...
export function SearchResults({
  ...
  const mailboxes = new Set(result?.items.map((item) => item.account_id) ?? [])
    .size;
  return (
    <>
