    ...
    <article className="printMessage">
      <header>
        ...
        <dl>
          ...
          <MetaRow
            label={t("mail.to")}
            value={message.to_emails.join(", ") || message.account_address}
          />
          {message.cc_emails.length > 0 && (
            <MetaRow
              label={t("mail.cc")}
              value={message.cc_emails.join(", ")}
            />
          )}
          <MetaRow
            label={t("mail.date")}
      ...
      {message.safe_html ? (
        ...
        <pre>{message.body_text || t("mail.emptyMessage")}</pre>
      )}
      {message.attachments.length > 0 && (
        <footer>
          <strong>{t("mail.attachmentsLabel")}:</strong>{" "}
