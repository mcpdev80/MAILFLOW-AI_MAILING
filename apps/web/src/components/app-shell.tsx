...
function useShellState(): ShellState {
  ...
  useEffect(() => {
    let active = true;
    Promise.allSettled([api.health(), attentionApi.review(), attentionApi.notifications()]).then(([health, review, notifications]) => {
      if (!active) return;
      setState({
  ...
  return (
    <aside className={styles.sidebar} data-testid="app-sidebar">
      <div className={styles.sidebarTop}>
        <Link href="/app/dashboard" className={styles.brand} aria-label="Mailflow"><span className={styles.brandMark}>M</span><span className={styles.brandText}>Mailflow</span></Link>
      ...
      <Link href="/app/settings/profile" className={styles.profile}><span className={styles.avatar}>{initials(user?.name, user?.email)}</span><span className={styles.profileMeta}><span className={styles.profileName}>{user?.name || user?.email || ""}</span>{user?.email && <span className={styles.profileEmail}>{user.email}</span>}</span><span className={styles.profileChevron} aria-hidden="true">⌄</span></Link>
    </aside>
  );
}
...
function StatusPill({ state }: { state: ShellState }) {
  ...
  return <div className={styles.statusWrap} data-testid="system-status"><span className={styles.statusLabel}>{t("shell.systemStatus")}:</span><span className={`${styles.status} ${state.healthy === false ? styles.statusDegraded : ""}`}><span className={styles.statusDot} aria-hidden="true" />{statusText}</span></div>;
}
