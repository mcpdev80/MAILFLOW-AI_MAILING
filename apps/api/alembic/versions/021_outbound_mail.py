...
def upgrade() -> None:
    op.add_column(
        "email_accounts", sa.Column("smtp_host", sa.String(length=255), nullable=True)
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "message_type", sa.String(length=16), nullable=False, server_default="new"
        ),
    ...
    op.create_index(
        "ix_outbound_drafts_account_status", "outbound_drafts", ["account_id", "status"]
    )
    op.create_index(
        "ix_outbound_drafts_owner_updated",
        "outbound_drafts",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["draft_id"], ["outbound_drafts.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_outbound_draft_attachments_draft",
        "outbound_draft_attachments",
        ["draft_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_outbound_draft_attachments_draft", table_name="outbound_draft_attachments"
    )
