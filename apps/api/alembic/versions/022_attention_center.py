...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "urgent_enabled", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.Column(
        ...
        sa.Column(
            "daily_summary_enabled", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.Column(
            "daily_summary_hour", sa.Integer(), nullable=False, server_default="8"
        ),
        sa.Column(
        ...
        sa.UniqueConstraint(
            "org_id", "user_key", name="uq_notification_preferences_actor"
        ),
        sa.CheckConstraint(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["source_email_id"], ["processed_emails.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "org_id", "user_key", "dedupe_key", name="uq_notification_event_dedupe"
        ),
    )
    op.create_index(
        "ix_notification_event_actor_read",
        "notification_events",
        ["org_id", "user_key", "read_at"],
    )
    op.create_index(
        "ix_notification_event_actor_created",
        "notification_events",
        ["org_id", "user_key", "created_at"],
    )


def downgrade() -> None:
    op.drop_index(
        "ix_notification_event_actor_created", table_name="notification_events"
    )
