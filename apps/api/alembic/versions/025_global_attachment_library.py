...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "managed_by", sa.String(length=16), server_default="ai", nullable=False
        ),
        sa.Column(
        ...
        sa.CheckConstraint(
            "managed_by IN ('ai', 'user')", name="ck_attachment_folders_managed_by"
        ),
        sa.ForeignKeyConstraint(["org_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["parent_id"], ["attachment_folders.id"], ondelete="CASCADE"
        ),
    ...
    op.create_index(
        "ix_attachment_folders_owner", "attachment_folders", ["org_id", "owner_scope"]
    )
    ...
    op.create_table(
        ...
        sa.Column("ai_tags", sa.JSON(), server_default="[]", nullable=False),
    ...
    op.create_index(
        "ix_attachment_documents_org_created",
        "attachment_documents",
        ["org_id", "created_at"],
    )
    op.create_index(
        "ix_attachment_documents_org_category",
        "attachment_documents",
        ["org_id", "ai_category"],
    )
    ...
    op.create_table(
        ...
        sa.Column("subcategory_override", sa.String(length=150), nullable=True),
        sa.Column("user_tags", sa.JSON(), server_default="[]", nullable=False),
        sa.Column(
            "corrected", sa.Boolean(), server_default=sa.text("false"), nullable=False
        ),
        sa.Column(
            "updated_at",
            sa.DateTime(timezone=True),
        ...
        sa.ForeignKeyConstraint(
            ["document_id"], ["attachment_documents.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
            ["folder_id"], ["attachment_folders.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["org_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
    ...
    op.create_index(
        "ix_attachment_placements_owner",
        "attachment_placements",
        ["org_id", "owner_scope"],
    )
    op.create_index(
        "ix_attachment_placements_folder", "attachment_placements", ["folder_id"]
    )
    ...
    op.create_index(
        "ix_attachment_sources_document", "attachment_sources", ["document_id"]
    )
    op.create_index(
        "ix_attachment_sources_account",
        "attachment_sources",
        ["account_id", "created_at"],
    )
    op.create_index(
        "ix_attachment_sources_status", "attachment_sources", ["ingestion_status"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["folder_id"], ["attachment_folders.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["org_id"], ["organizations.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_attachment_memory_owner",
        "attachment_memory",
        ["org_id", "owner_scope", "active"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index("ix_attachment_placements_folder", table_name="attachment_placements")
    op.drop_index("ix_attachment_placements_owner", table_name="attachment_placements")
    op.drop_table("attachment_placements")
    op.drop_index(
        "ix_attachment_documents_org_category", table_name="attachment_documents"
    )
    op.drop_index(
        "ix_attachment_documents_org_created", table_name="attachment_documents"
    )
